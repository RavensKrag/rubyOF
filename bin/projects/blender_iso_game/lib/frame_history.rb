class FrameHistory
  ...
  module States
    ...
    class Generating_New < State
      ...
      def play
        # TODO: consider moving @paused into the context object, so that Generating_New and Replaying_Old can share the same paused state
        @paused = false
      end
      ...
      def step_forward
        @outer.instance_eval do
          ...
          if @f1.alive?
            ...
            self.state = :finished
            # NOTE: ^ one dropped frame between the final state and transitioning to :finished
          end
      ...
      def seek(frame_number)
        ...
        @outer.instance_eval do
          ...
          # transition to self.state = :replaying_old
          
          # but maybe should check if the transition is valid first? what if you tried to seek to a frame that has not yet been generated?
    ...
    class Replaying_Old < State
      ...
      def update
        ...
        step_forward()
        # NOTE: ^ one dropped frame between replaying history and resuming new state, as the system needs to update the active state
      end
      ...
      def seek(frame_number)
        ...
        @outer.instance_eval do
          ...
          if frame_number >= 0 && frame_number < @history.length
    ...
    # (backwards via stored history. fiber set in FrameHistory#setup_states)
    class Reverse < State
      def update
        @outer.instance_eval do
