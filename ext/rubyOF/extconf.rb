...
# TODO: Update library loads such that there is one line where you can change the directory of the custom dependency files.
...
# NOTE: The lower 'headers' line works on Ubuntu 16.04, but the upper headers line worked on 16.10 IIRC. Probably needs further testing. However, as this build currently targets Ubuntu 16.04, this will be ignored for now.
# TODO: Make sure this build process works with later versions of Ubuntu / other operating systems in general.

# headers = File.expand_path("../of_v0.9.3_libs/custom/#{dependency_name}/include/#{dependency_name.upcase}", OF_ROOT)
headers = File.expand_path("#{dependency_name}/include/", OF_CUSTOM_DEPS_PATH)
libs    = File.expand_path("#{dependency_name}/lib/linux64", OF_CUSTOM_DEPS_PATH)
...
have_header("GLFW/glfw3.h")   # oF version
...
category_list.each do |category|
	...
	poco_root = File.expand_path("poco/poco-1.7.4-all/", OF_CUSTOM_DEPS_PATH)
...
dir_config(
	dependency_name, 
	File.expand_path("tess2/Include/", OF_CUSTOM_DEPS_PATH),
	File.expand_path("tess2/Build/",   OF_CUSTOM_DEPS_PATH)
)
...
headers = [
	File.expand_path("kiss/",        OF_CUSTOM_DEPS_PATH),
	File.expand_path("kiss/tools/",  OF_CUSTOM_DEPS_PATH),
]
libs = File.expand_path("kiss/", OF_CUSTOM_DEPS_PATH)
...
dependency_name = "utf8"

headers = [
	File.expand_path("./libs/utf8/include", OF_ROOT),
	File.expand_path("./libs/utf8/include/utf8", OF_ROOT),
]
